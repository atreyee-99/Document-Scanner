...
thres = [50, 50]
imgThreshold = cv2.Canny(imgBlur, thres[0], thres[1])  # APPLY CANNY BLUR #gives edges
kernel = np.ones((5, 5)) #creates 5*5 matrix whose values are 1 #np-numPy
...
if biggest.size != 0:
    ...
    imgAdaptiveThre = cv2.bitwise_not(imgAdaptiveThre) #binary image
